...
# Base tools
sudo apt install -y openssh-server linux-tools-generic git curl wget btop htop neofetch gnupg2 gpg
...
fish -c "echo 'zoxide init fish | source' >> ~/.config/fish/config.fish"
...
fish -c "echo 'alias cd='z'' >> ~/.config/fish/config.fish"
...
# Time for login and setup services
